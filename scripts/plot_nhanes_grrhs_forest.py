...
OUT_DIR = ROOT / "outputs" / "figures" / "nhanes_grrhs"
PNG_PATH = OUT_DIR / "nhanes_grrhs_forest.png"
PDF_PATH = OUT_DIR / "nhanes_grrhs_forest.pdf"
CSV_PATH = OUT_DIR / "nhanes_grrhs_effect_summary.csv"
JSON_PATH = OUT_DIR / "nhanes_grrhs_fit_summary.json"
...
def _load_inputs() -> tuple[
    ...
    return x_log, y_log, c_model, np.asarray(bundle["group_index_exposure"], dtype=int), groups, feature_codes, feature_labels, feature_groups
...
def _plot(
    *,
    ...
    for idx, (label, group, mean, lo, hi) in enumerate(zip(feature_labels, feature_groups, means, lower, upper)):
        ypos = y_pos[idx]
    ...
    ax.set_title("NHANES 2003-2004: GR-RHS Exposure Effects", fontsize=14, pad=12, fontweight="bold")
    ax.text(
        0.0,
        1.01,
        "Posterior mean with 95% credible interval; fitted after full-sample covariate residualization on log-transformed data",
...
def main() -> None:
    ...
    x_log, y_log, c_model, _group_index, groups, feature_codes, feature_labels, feature_groups = _load_inputs()
    result, x_used, y_used, x_scale = _fit_model(x_log, y_log, c_model, groups)
    if result.beta_draws is None:
        raise RuntimeError(f"GR_RHS fit did not return coefficient draws: {result.error}")
    ...
    JSON_PATH.write_text(json.dumps(fit_summary, indent=2), encoding="utf-8")
