...
for rr in [6,12,20]:
    m=GRRHS_Gibbs(c=1.0,tau0=0.1,eta=0.5,alpha_c=2.0,beta_c=2.0,s0=1.0,
                  ...
                  tau2_refresh_steps=rr,use_tau_slice_refresh=True)
    t0=time.perf_counter(); m.fit(X,y,groups=groups); dt=time.perf_counter()-t0
    arrays={'beta':np.asarray(m.coef_samples_),'tau':np.asarray(m.tau_samples_),'a':np.asarray(m.a_samples_),'c2':np.asarray(m.c2_samples_),'lambda':np.asarray(m.lambda_samples_)}
    s=summarize_convergence(arrays,min_chains_for_rhat=int(conv_cfg.get('min_chains_for_rhat',4)))
    ok,fail=_check_convergence(s,conv_cfg,model_name='grrhs_gibbs',sampler_diagnostics=m.sampler_diagnostics_)
    print(f'refresh={rr} time={dt:.2f}s ok={ok} beta_ess={s["beta"].get("ess_min"):.1f} tau_ess={s["tau"].get("ess_min"):.1f} tau_rhat={s["tau"].get("rhat_max"):.4f}')
    if not ok:
