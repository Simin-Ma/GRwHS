...
def bench(burnin, kept):
    ...
    print(f"iters={iters} (burnin={burnin}, kept={kept}) -> {elapsed:.2f}s, converged={ok}")
    print(f"  beta_rhat={summary['beta']['rhat_max']:.4f}, beta_ess={summary['beta']['ess_min']:.1f}, tau_ess={summary['tau']['ess_min']:.1f}")
    if not ok:
        print('  failures:', '; '.join(failures[:6]))
