    ...
    def _build_model(
        ...
        def model(X, y, group_id, group_sizes, tau0_eff):
            ...
            if profile_mode and group_XXT is not None:
                # lam=1, a=1 → v only depends on group: v_g = sigma2*kappa_g*tau2/(sigma2*kappa_g+(1-kappa_g)*tau2)
    ...
    def fit(
        ...
        if profile_mode:
            ...
            for g_idx, members in enumerate(groups_use):
                ...
                M_list.append(Xg @ Xg.T)  # n×n
            group_XXT_jnp = jnp.asarray(np.stack(M_list, axis=0))  # G×n×n
