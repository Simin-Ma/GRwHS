...
def _auto_inject_tau0(resolved_cfg: Dict[str, Any], *, n: int | None = None, p: int | None = None) -> Dict[str, Any]:
    """
    If model is GRRHS and tau0 not provided, set tau0 using the heuristic:
    unit_variance: tau0 = (s / (p - s)) / sqrt(n)
    unit_l2:       tau0 =  s / (p - s)
    """
    model = resolved_cfg.get("model", {})
    name = str(model.get("name", "")).lower()
    if not name.startswith("grrhs"):
        return resolved_cfg
...
def _maybe_call_runner(resolved_cfg: Dict[str, Any], run_dir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            "[WARN] grrhs.experiments.runner.run_experiment not found yet. "
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run a single GRRHS experiment from YAML config.",
