...
def _sample_profile_signal_group(
    ...
    *,
    ...
    if str(signal_type).lower() == "concentrated":
...
def _make_benchmark_dataset(setting: str, *, seed: int, n: int = 500, snr: float = 0.7, rho_between: float = 0.1) -> Dict[str, Any]:
    ...
    X = _sample_design(n=n, groups=groups, rho_within=cfg["rho_within"], rho_between=rho_between, rng=rng)
...
def _make_heterogeneity_dataset(seed: int) -> Dict[str, Any]:
    rng = np.random.default_rng(seed)
    groups = _canonical_groups([10] * 6)
    X = _sample_design(n=300, groups=groups, rho_within=0.7, rho_between=0.05, rng=rng)
    ...
    return {"X": X, "y": y, "beta": beta, "groups": groups, "mu": np.asarray(mu, dtype=float), "task": "gaussian"}
...
def _make_grouped_logistic_dataset(seed: int) -> Dict[str, Any]:
    rng = np.random.default_rng(seed)
    groups = _canonical_groups([5, 5, 5])
    X = _sample_design(n=50, groups=groups, rho_within=0.5, rho_between=0.05, rng=rng)
    beta = np.zeros(X.shape[1], dtype=float)
    beta[0] = 3.0
    beta[1] = 3.0
    beta[10] = 0.5
...
def _run_theory_section(outdir: Path, *, quick: bool, seed: int) -> Dict[str, Any]:
    ...
    phase_xi = [0.01, 0.05, 0.2] if quick else [0.005, 0.01, 0.03, 0.05, 0.1, 0.2, 0.5]
    phase_pg = [30, 60] if quick else [30, 60, 120]
    adaptive_band = {"x_lower": 0.3, "x_upper": 3.0}

    null_rows: list[dict[str, float]] = []
    for p_g in null_grid:
        thresh = 2.0 / math.sqrt(float(p_g))
        posterior_means = []
        tail_probs = []
        for _ in range(reps):
            ...
            posterior_means.append(float(np.trapezoid(obj["kappa"] * obj["density"], obj["kappa"])))
            mask = obj["kappa"] > thresh
            tail_probs.append(float(np.trapezoid(obj["density"][mask], obj["kappa"][mask])) if np.any(mask) else 0.0)
        null_rows.append({"p_g": float(p_g), "median_posterior_mean": float(np.median(posterior_means)), "mean_tail_prob": float(np.mean(tail_probs))})
    ...
    for p_g in adaptive_grid:
        ...
        for _ in range(reps):
            ...
            obj = profile_kappa_log_posterior_grid(ds["y_group"], tau=0.1, sigma=1.0, alpha_kappa=1.0, beta_kappa=1.0)
        ...
        adaptive_rows.append({"p_g": float(p_g), "ratio_median": float(np.nanmedian(ratios)), "ratio_iqr_low": float(np.nanquantile(ratios, 0.25)), "ratio_iqr_high": float(np.nanquantile(ratios, 0.75))})
        adaptive_appendix_rows.append({"p_g": float(p_g), "appendix_band_x_lower": float(adaptive_band["x_lower"]), "appendix_band_x_upper": float(adaptive_band["x_upper"]), "appendix_band_mass_mean": float(np.nanmean(band_mass))})
    ...
    xi_crit = 0.5 * (-math.log(max(1.0 - 0.5, 1e-8)))
    ...
    for xi_inf in phase_xi:
        for p_g in phase_pg:
            ...
            for _ in range(max(2, reps // 2)):
                mu = float(xi_inf) * float(p_g)
                ds = _sample_profile_signal_group(p_g, mu, seed=int(rng.integers(1_000_000)), sigma=1.0, signal_type="distributed")
                obj = profile_kappa_log_posterior_grid(ds["y_group"], tau=0.3, sigma=1.0, alpha_kappa=1.0, beta_kappa=1.0)
                mask = obj["kappa"] > 0.5
                probs.append(float(np.trapezoid(obj["density"][mask], obj["kappa"][mask])) if np.any(mask) else 0.0)
            phase_rows.append({"xi_inf": float(xi_inf), "p_g": float(p_g), "prob_kappa_gt_u0": float(np.nanmean(probs)), "xi_crit": float(xi_crit)})
    ...
    manifest = {
        ...
        "strong_signal_u0": 0.5,
        "strong_signal_rho_tau_over_sigma": 0.3,
    }
...
def _run_benchmark_section(outdir: Path, *, quick: bool, seed: int, methods: Sequence[str]) -> Dict[str, Any]:
    ...
    for setting in [f"S{k}" for k in range(1, 13)]:
        for rep in range(reps):
            ...
            for method, fit in fits.items():
                ...
                rows.append({"setting": setting, "rep": rep, "method": method, "status": "ok", "CI_length": width, "Coverage": coverage, **_mse_partition(ds["beta"], beta_hat), "convergence_passed": bool(conv["passed"]), "max_rhat": conv["max_rhat"], "min_bulk_ess": conv["min_bulk_ess"], "divergence_rate": conv["divergence_rate"]})
...
def _run_heterogeneity_section(outdir: Path, *, quick: bool, seed: int, methods: Sequence[str]) -> Dict[str, Any]:
    ...
    for rep in range(reps):
        ...
        fits = _fit_many_methods(subset, ds["X"], ds["y"], ds["groups"], task="gaussian", base_seed=int(rng.integers(1_000_000)), quick=quick, p0=4)
        for method, fit in fits.items():
            if isinstance(fit, Exception):
                continue
            posterior = _posterior_draws(fit)
            beta_hat = _posterior_mean(posterior.get("beta"))
            mse_rows.append({"rep": rep, "method": method, **_mse_partition(ds["beta"], beta_hat)})
            scores = _group_scores_from_model(fit, ds["groups"])
            ...
            for gid, mu_g in enumerate(ds["mu"]):
                ...
                if is_grrhs_variant and "kappa_mean" in scores:
                    mono_rows.append({"rep": rep, "group_id": gid, "group_signal_mu": float(mu_g), "kappa_mean": float(scores["kappa_mean"][gid])})
...
def _run_inferential_section(outdir: Path, *, quick: bool, seed: int, methods: Sequence[str]) -> Dict[str, Any]:
    ...
    return {"auroc_table": str(outdir / "inferential_auroc.csv"), "bridge_table": str(outdir / "inferential_bridge.csv")}
...
def _run_logistic_section(outdir: Path, *, quick: bool, seed: int, methods: Sequence[str]) -> Dict[str, Any]:
    ...
    for rep in range(reps):
        ds = _make_grouped_logistic_dataset(seed=int(rng.integers(1_000_000)))
    ...
    return {"table": str(outdir / "logistic_weak_identification.csv")}
...
def _run_hyperparameter_section(outdir: Path, *, quick: bool, seed: int) -> Dict[str, Any]:
    profile = RevisionQuickProfile()
    reps = profile.hyper_reps if quick else 400
    rng = np.random.default_rng(seed)
    ...
    for rep in range(reps):
        ...
        for alpha_kappa, beta_kappa in [(0.5, 0.5), (1.0, 1.0), (1.0, 2.0), (0.5, 1.0)]:
            ...
            scores = _group_scores_from_model(fit, ds["groups"])
            beta_hat = _posterior_mean(_posterior_draws(fit).get("beta"))
            rows.append({"rep": rep, "alpha_kappa": alpha_kappa, "beta_kappa": beta_kappa, **_mse_partition(ds["beta"], beta_hat), "kappa_auroc": float(roc_auc_score(labels, scores["kappa_mean"])) if "kappa_mean" in scores else float("nan")})
    pd.DataFrame(rows).to_csv(outdir / "hyper_beta_prior_grid.csv", index=False)
    return {"tau_table": str(outdir / "hyper_tau_meff.csv"), "beta_grid_table": str(outdir / "hyper_beta_prior_grid.csv")}
