"""Validation checklist harness for GRwHS."""
...
from grwhs.diagnostics.postprocess import DiagnosticsResult, compute_diagnostics_from_samples
...
class ValidationChecklistRunner:
    """Orchestrates checklist scenarios for GRwHS."""
    ...
    def _s2_group_phi_sensitivity(self) -> ScenarioOutcome:
        ...
        for eta in etas:
            fit = self._fit_grwhs(ds, eta=eta, use_groups=True, iters=520 if not self.fast else 320, burnin=170)
    ...
    def _s3_local_lambda_sensitivity(self) -> ScenarioOutcome:
        ...
        for sf in scale_factors:
            fit = self._fit_grwhs(ds, use_groups=True, iters=780 if not self.fast else 500, burnin=230 if not self.fast else 200)
    ...
    def _s4_slab_sensitivity(self) -> ScenarioOutcome:
        ...
        for c_val in c_values:
            fit = self._fit_grwhs(ds, c=c_val, eta=0.6, use_groups=True, iters=520 if not self.fast else 320, burnin=170)
    ...
    def _nc_dense_weak(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit_grwhs = self._fit_grwhs(ds, eta=0.4, use_groups=True, iters=1000 if not self.fast else 650, burnin=300)
        fit_rhs = self._fit_grwhs(ds, eta=0.6, use_groups=False, iters=520 if not self.fast else 320, burnin=170)
        diff = self._compare_runs(fit_grwhs, fit_rhs)

        XtX = fit_grwhs.X.T @ fit_grwhs.X
        alpha = 1.0
        coef_ridge = np.linalg.solve(XtX + alpha * np.eye(XtX.shape[0]), fit_grwhs.X.T @ fit_grwhs.y)
        pred_ridge = fit_grwhs.X @ coef_ridge
        rmse_ridge = _rmse(fit_grwhs.y, pred_ridge)

        status = "pass"
        notes: List[str] = []
        tol = 0.35
        ridge_gap = abs(rmse_ridge - fit_grwhs.rmse)
        if diff.get("rmse_gap", 0.0) < tol and ridge_gap < tol:
            notes.append("GRwHS appropriately matches RHS/Ridge in dense-weak regime.")
        else:
            status = "warn" if diff.get("rmse_gap", 0.0) > tol or ridge_gap > tol else "pass"
            if status == "warn":
                notes.append("GRwHS deviated from RHS/Ridge on dense-weak control.")
            else:
                notes.append("Minor deviation vs RHS/Ridge but within tolerance.")

        metrics = {
            "rmse_grwhs": fit_grwhs.rmse,
            "rmse_rhs": fit_rhs.rmse,
            "rmse_ridge": rmse_ridge,
            "rmse_gap_grwhs_rhs": diff.get("rmse_gap", float("nan")),
        }
        return ScenarioOutcome(
            key="NC-1",
    ...
    def _nc_group_misspec(self) -> ScenarioOutcome:
        ...
        fit_true = self._fit_grwhs(ds, eta=0.6, use_groups=True, iters=520 if not self.fast else 320, burnin=170)
        fit_bad = self._fit_grwhs(ds_bad, eta=0.6, use_groups=True, iters=520 if not self.fast else 320, burnin=170)
        # Strict rerun to check if fast WARN is an artefact
        fit_bad_strict = self._fit_grwhs(
            ds_bad,
            eta=0.6,
            use_groups=True,
        ...
        return ScenarioOutcome(
            ...
            metrics=metrics,
            expectations=[
                "Mild degradation only; GRwHS should fall back toward RHS.",
    ...
    def _e1_phi_vs_true_strength(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit = self._fit_grwhs(ds, eta=0.6, use_groups=True, iters=520 if not self.fast else 320, burnin=170)
        phi_med = np.median(fit.posterior["phi"], axis=0)
        group_norms = []
    ...
    def _e2_group_order_stability(self) -> ScenarioOutcome:
        ...
        for s in seeds:
            fit = self._fit_grwhs(ds, eta=0.6, use_groups=True, iters=520 if not self.fast else 360, burnin=170, seed=s)
            phi_med = np.median(fit.posterior["phi"], axis=0)
    ...
    def _failure_modes(self) -> ScenarioOutcome:
        ...
        notes = [
            ...
            "Expect GRwHS to at best match RHS; monitor for degradation.",
        ]
        return ScenarioOutcome(
            key="FailureModes",
            ...
            metrics=metrics,
            expectations=[
                "GRwHS should not outperform RHS when groups are indistinguishable or highly correlated.",
    ...
    def _sc_no_group_structure(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit_grouped = self._fit_grwhs(ds, eta=0.5, use_groups=True, iters=700 if not self.fast else 400, burnin=250)
        fit_rhs = self._fit_grwhs(ds, eta=0.5, use_groups=False, iters=700 if not self.fast else 400, burnin=250)
        ...
        if diff.get("rmse_gap", 0.0) > 0.25:
            status = "warn"
            notes.append("GRwHS predictive gap vs RHS is larger than expected when groups are uninformative.")
    ...
    def _sc_single_strong_signal(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit = self._fit_grwhs(ds, c=1.5, tau0=0.15, eta=0.5, use_groups=True, iters=900 if not self.fast else 450, burnin=300)
        metrics = self._basic_metrics(fit)
        notes: List[str] = []
        status = "pass"
    ...
    def _d1_degeneration_rhs(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit_g = self._fit_grwhs(ds, eta=0.6, use_groups=True, iters=800 if not self.fast else 450, burnin=250)
        fit_rhs = self._fit_grwhs(ds, eta=0.6, use_groups=False, iters=800 if not self.fast else 450, burnin=250)
    ...
    def _d3_local_collapse(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit = self._fit_grwhs(ds, eta=0.6, use_groups=True, iters=750 if not self.fast else 420, burnin=240)
    ...
    def _d4_slab_extremes(self) -> ScenarioOutcome:
        ...
        ds = _build_dataset(cfg)
        fit_lo = self._fit_grwhs(ds, c=0.5, eta=0.6, use_groups=True, iters=650 if not self.fast else 380, burnin=220)
        fit_hi = self._fit_grwhs(ds, c=50.0, eta=0.6, use_groups=True, iters=650 if not self.fast else 380, burnin=220)
    ...
    def _s1_tau_sensitivity(self) -> ScenarioOutcome:
        ...
        for scale in tau_scales:
            fit = self._fit_grwhs(ds, tau0=0.2 * scale, eta=0.6, use_groups=True, iters=520 if not self.fast else 320, burnin=180)
            rmses.append(fit.rmse)
