"""Logistic GRwHS Gibbs sampler with Pólya–Gamma augmentation."""
...
from grwhs.inference.pg import sample_pg
from grwhs.models.grwhs_gibbs import (
    GRwHS_Gibbs,
    _BURNIN_WARM,
    _PHI_ADAPT_COEFF,
    _PHI_BASE_FLOOR,
...
class GRwHS_Gibbs_Logistic(GRwHS_Gibbs):
    """Group-regularized Horseshoe Gibbs sampler for Bernoulli-logit likelihoods."""

    def fit(self, X: np.ndarray, y: np.ndarray, groups: Optional[List[List[int]]] = None) -> "GRwHS_Gibbs_Logistic":
        """
        ...
        if np.any((y < -1e-8) | (y > 1 + 1e-8)):
            raise ValueError("Logistic GRwHS requires binary labels in {0, 1}.")
        y = np.clip(y, 0.0, 1.0)
