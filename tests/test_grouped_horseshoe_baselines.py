...
def test_grouped_horseshoe_plus_fit_exposes_group_level_draws():
    X, y, beta_true, groups = _toy_grouped_dataset()
    model = GroupedHorseshoePlus(iters=80, burnin=30, thin=2, seed=1, num_chains=1)
    fitted = model.fit(X, y, groups=groups)

    assert fitted.coef_.shape == beta_true.shape
    assert fitted.group_lambda_samples_.shape[1] == len(groups)
...
def test_grouped_horseshoe_plus_registry_builder_runs():
    X, y, beta_true, groups = _toy_grouped_dataset(seed=7)
    cfg = {
        "model": {
            "name": "grouped_horseshoe_plus",
            "fit_intercept": True,
            "tau0": 0.5,
            "iters": 80,
            "burnin": 30,
    ...
    assert isinstance(model, GroupedHorseshoePlus)
    fitted = model.fit(X, y, groups=groups)

    assert fitted.coef_.shape == beta_true.shape
    assert fitted.group_lambda_mean_.shape[0] == len(groups)
    assert fitted.lambda_mean_.shape[0] == X.shape[1]
    assert fitted.sampler_diagnostics_["model"] == "HBGHS (Xu et al. 2016)"
