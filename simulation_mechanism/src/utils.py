...
def stringify_groups(group_sizes: Sequence[int]) -> str:
    return "[" + ",".join(str(int(v)) for v in group_sizes) + "]"


def mechanism_method_label(name: str) -> str:
    if str(name) in _METHOD_LABEL_OVERRIDES:
        return _METHOD_LABEL_OVERRIDES[str(name)]
    label = method_result_label(str(name))
    if label == str(name):
        return method_display_name(str(name))
    return label


def mechanism_method_family(name: str) -> str:
    text = str(name)
    if text == "RHS_oracle":
        return "RHS"
